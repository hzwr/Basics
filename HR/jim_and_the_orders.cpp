...
vector<int> jimOrders(const vector<vector<int>> &orders) {
    multimap<int, int> customerNumberOfServeTime;
    
    int customer = 1;
    for(auto order : orders)
    {
        customerNumberOfServeTime.insert({order[0]+order[1], customer});
        ++customer;
    }
    
    vector<int> customerNumbers;
    for(auto customer : customerNumberOfServeTime)
    {
        customerNumbers.emplace_back(customer.second);
    }
